...
export default function PostDetailScreen({ route }: any) {
    ...
    const [showDeleteComment, setShowDeleteComment] = useState<{ visible: boolean; id?: string }>({ visible: false });
            ...
            .onSnapshot(async (doc) => {
                if (doc.exists()) {
                    ...
                    if (data?.uid) {
                        ...
                        setAuthor(userDoc.exists() ? userDoc.data()?.nickname : "알 수 없음");
                    }
                }
            });
        ...
        const unsubComments = firestore()
            ...
            .onSnapshot(async (snapshot) => {
                const commentData = snapshot.docs.map(
                    doc => ({ ...(doc.data() as Omit<Comment, "id">), id: doc.id })
                );

                // 작성자 닉네임 매핑
                const userIds = Array.from(new Set(commentData.map((c: any) => c.uid).filter(Boolean)));
                const userDocs = await Promise.all(userIds.map(uid => firestore().collection("users").doc(uid).get()));
                const userMap: Record<string, string> = {};
                userDocs.forEach(doc => {
                    if (doc.exists()) userMap[doc.id] = doc.data()?.nickname || "알 수 없음";
                });
    ...
    const saveEdit = async (id: string) => {
        try {
            await firestore().collection("posts").doc(postId).collection("comments").doc(id).update({
                text: editText.trim(),
                updatedAt: firestore.FieldValue.serverTimestamp(),
            });
            setEditingId(null);
    ...
    const handleDeleteComment = async (id: string) => {
        try {
            await firestore().collection("posts").doc(postId).collection("comments").doc(id).delete();
            await firestore().collection("posts").doc(postId).update({
                commentCount: firestore.FieldValue.increment(-1),
            });
        } catch (err) {
    ...
    return (
        <View style={styles.container}>
                        ...
                        <View style={styles.commentRow}>
                            ...
                            <View style={{ flex: 1 }}>
                                <Text style={styles.commentMeta}>
                                    ...
                                    {item.updatedAt?.toDate
                                        ...
                                        : "로딩중..."}{" "}
                                    {item.updatedAt && item.updatedAt.toDate()?.getTime() !== item.createdAt?.toDate()?.getTime()
                                        ? "(수정됨)"
                                        : ""}
                                </Text>
                                {isEditing ? (
                            ...
                            {isMine && !isEditing && (
                                <View style={styles.actionTextRow}>
                                    ...
                                    <TouchableOpacity onPress={() =>  setShowDeleteComment({ visible: true, id: item.id })}>
            ...
            <ConfirmModal
                ...
                confirmText="삭제"
                cancelText="취소"
                onConfirm={() => showDeleteComment.id && handleDeleteComment(showDeleteComment.id)}
...
const styles = StyleSheet.create({
    ...
    meta: { fontSize: 12, color: "#777", marginBottom: 10, lineHeight: 18 },
    content: { fontSize: 15, marginBottom: 10, color: "#333", lineHeight: 22 },
    image: {
        width: "100%",
        height: 200,
        resizeMode: "cover",
