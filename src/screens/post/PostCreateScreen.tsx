...
export default function PostCreateScreen({ }: any) {
    ...
    // 게시글 작성
    const handleCreate = async ({ title, content, image }: NewPostData) => {
        setLoading(true);
        try {
            let imageUrl = "";
            if (image?.uri) {
            ...
            await firestore().runTransaction(async (transaction) => {
                ...
                const current = counterDoc.exists() ? counterDoc.data()?.postCounter || 0 : 0;
                const newNumber = current + 1;

                transaction.set(counterRef, { postCounter: newNumber }, { merge: true });
    ...
    return (
        <View style={{ flex: 1 }}>
            <PostForm onSubmit={handleCreate} submitLabel="작성 완료" loading={loading}/>
